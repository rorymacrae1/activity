 ...
 * LocationMapSection
 *
 * Shows an OpenStreetMap-based view centred on the resort village, with
 * colour-coded pins for nearby accommodations and ski shops / rentals.
 *
 * · Preview card (non-interactive) — always visible on the resort page.
 * · "Explore Map" tap → full-screen modal with interactive Leaflet map.
 * · POI data is loaded from the Overpass API when online; fails silently
 *   when offline (only the resort-centre pin is shown).
 *
 * Colours are locked to theme hex values so they render correctly inside
 * the WebView HTML without needing to import theme modules at runtime.
 */
...
/**
 * Embeds an OpenStreetMap preview on the resort detail page.
  ...
  return (
    <View style={styles.section}>
      ...
      <View style={styles.previewContainer}>
        ...
        <WebView
          style={[styles.previewMap, !previewReady && styles.invisible]}
          source={{ html: previewHtml }}
          scrollEnabled={false}
          bounces={false}
          onLoad={() => setPreviewReady(true)}
          accessibilityLabel={`${resort.name} location map preview`}
          accessibilityHint="Tap Explore Map to open an interactive full-screen map"
        />
          ...
          {!modalReady && (
            <View style={styles.modalLoader}>
              ...
              <Text
                variant="bodySmall"
                color={colors.ink.muted}
                style={styles.modalLoaderText}
              >
                Loading map & nearby places…
              </Text>
            </View>
          )}
          <WebView
            style={[styles.fullMap, !modalReady && styles.invisible]}
            source={{ html: fullHtml }}
            onLoad={() => setModalReady(true)}
            accessibilityLabel={`Interactive map of ${resort.name}`}
            accessibilityHint="Pinch to zoom, drag to pan the map"
          />
